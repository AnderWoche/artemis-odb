...
public class WorldTest {
    ...
    public static class AnComponent extends PooledComponent {
        public int id;
        public String name;
        @Override
    ...
    @All()
    public static class VoidSys extends IteratingSystem {

        private ComponentMapper<AnComponent> mapper;
        ...
        public ComponentMapper<AnComponent> getMapper() {
            return this.mapper;
        }
    }

    @Test
    public void test_world_injection() {

        VoidSys voidSys = new VoidSys();

        World w = new World();
        w.createEntity().edit().create(AnComponent.class);

        World w2 = new World();
        w2.createEntity().edit().create(AnComponent.class);
        w2.createEntity().edit().create(AnComponent.class);

        w2.inject(voidSys);

        assertNotNull(voidSys.getMapper());
        assertTrue(voidSys.getMapper().has(0));
        assertTrue(voidSys.getMapper().has(1));

        w.inject(voidSys);

        assertNotNull(voidSys.getMapper());
        assertTrue(voidSys.getMapper().has(0));
        assertFalse(voidSys.getMapper().has(1));
